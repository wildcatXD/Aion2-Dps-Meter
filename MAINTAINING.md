...
파서를 먼저 만들지 않습니다. 캡처가 오면 `StreamProcessor`에만 옵코드를 넣고, 이미 있는 `DataManager.odeEnergy` / `shugoKeys` / 추적 오버레이에 숫자를 연결합니다. 오드에너지는 1.9.9에서 `0x0B/0x0C 0x61`을 붙였습니다. 허수아비 이름은 1.9.10에서 스폰을 놓쳐도 전투 중 카탈로그 코드·UTF-8 이름을 보조탐지합니다. 전투 기록 목록 자동 갱신·기록 선택 고정은 1.9.11입니다.

| 상황 | 남길 것 |
| --- | --- |
| 슈고페스타 열쇠 | 획득·사용·인벤 갯수가 보일 때, `app.log` 해당 구간 |
